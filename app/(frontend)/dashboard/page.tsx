...
import { getMyTrainingAssignments } from "@/lib/training/user-view";
...
export default async function DashboardPage() {
  ...
  // Pflichtschulungen (ADR 0005, Phase 3): kleiner Einstiegspunkt, der NUR
  // erscheint, wenn dem User etwas Unerledigtes zugewiesen ist — kein
  // Menüpunkt/Kachel für Leute ohne Pflichten.
  const trainingItems = await getMyTrainingAssignments(user.id);
  const pendingTraining = trainingItems.filter(
    (i) => i.ampel.status !== "erledigt",
  );
  const overdueTraining = pendingTraining.filter(
    (i) => i.ampel.status === "ueberfaellig",
  ).length;
  const soonTraining = pendingTraining.filter(
    (i) => i.ampel.status === "faellig_bald",
  ).length;
  const openTraining = pendingTraining.length - overdueTraining - soonTraining;
      ...
      <main className={styles.wrap}>
        ...
        {pendingTraining.length > 0 && (
          <section
            ...
            data-severity={overdueTraining > 0 ? "danger" : "warning"}
          >
            <div className={styles.complianceKicker}>Pflichtschulungen</div>
            <h2 className={styles.complianceTitle}>
              {overdueTraining > 0
                ? `${overdueTraining} ${overdueTraining === 1 ? "Pflichtschulung" : "Pflichtschulungen"} überfällig`
                : `${pendingTraining.length} ${pendingTraining.length === 1 ? "Pflichtschulung" : "Pflichtschulungen"} offen`}
            </h2>
            <div className={styles.complianceMeta}>
              {[
                overdueTraining > 0 &&
                  `${overdueTraining} überfällig`,
                soonTraining > 0 && `${soonTraining} bald fällig`,
                openTraining > 0 && `${openTraining} offen`,
              ]
                .filter(Boolean)
                .join(" · ")}
            </div>
            <Link href="/meine-pflichtschulungen" className="btn btn-primary">
              Zur Übersicht →
            </Link>
          </section>
        )}
