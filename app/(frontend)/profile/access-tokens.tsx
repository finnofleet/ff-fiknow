...
export function AccessTokensManager({
  ...
  /** Voller MCP-Endpoint (Streamable-HTTP), z. B. https://host/api/mcp/mcp */
  mcpUrl: string;
  /** true, wenn mcpUrl nur ein Platzhalter ist (kein Host ermittelbar) */
  mcpUrlIsPlaceholder: boolean;
}) {
  ...
  return (
    <div>
      ...
      <p className={styles.tokenHint}>
        ...
        ohne Browser-Session. Trage Endpoint-URL und Token so in deiner
        MCP-Client-Config (z. B. Cursor, Claude Desktop) ein:
      </p>
      {mcpUrlIsPlaceholder && (
        <p className={styles.tokenPlaceholderNotice}>
          Kein öffentlicher Host ermittelbar — <code>AUTHORING_PUBLIC_URL</code>{" "}
          im Deployment setzen, damit hier die echte Endpoint-URL erscheint.
        </p>
      )}
