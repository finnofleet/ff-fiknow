...
/**
 * Öffentliche Basis-URL für die MCP-Client-Config — spiegelt `publicOrigin()`
 * aus app/(frontend)/api/mcp/[transport]/route.ts (Zeile ~108), damit die hier
 * angezeigte URL exakt zum tatsächlich erreichbaren Endpoint passt.
 *
 * Reihenfolge: AUTHORING_PUBLIC_URL (Env-Override, gewinnt immer) → Proxy-
 * Header (x-forwarded-host/-proto) → kein geratener Wert, sondern Platzhalter.
 */
async function resolveMcpOrigin(): Promise<{
  origin: string;
  isPlaceholder: boolean;
}> {
  const override = process.env.AUTHORING_PUBLIC_URL?.trim();
  if (override) {
    return { origin: override.replace(/\/+$/, ""), isPlaceholder: false };
  }

  const hdrs = await headers();
  const host = hdrs.get("x-forwarded-host") || hdrs.get("host");
  if (!host) {
    return { origin: "https://<deine-domain>", isPlaceholder: true };
  }
  const proto = hdrs.get("x-forwarded-proto") || "https";
  return { origin: `${proto}://${host}`, isPlaceholder: false };
}
...
export default async function ProfilePage() {
  ...
  if (canManageCourses(role)) {
    ...
    const { origin, isPlaceholder } = await resolveMcpOrigin();
    mcpUrl = `${origin}/api/mcp/mcp`;
    mcpUrlIsPlaceholder = isPlaceholder;
  }
      ...
      <main className={styles.wrap}>
        ...
        {canManageCourses(role) && (
          <section className={styles.section}>
            ...
            <AccessTokensManager
              initialTokens={authoringTokens}
              mcpUrl={mcpUrl}
              mcpUrlIsPlaceholder={mcpUrlIsPlaceholder}
            />
          </section>
        )}
