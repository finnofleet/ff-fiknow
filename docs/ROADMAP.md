...
| **Pflichtkurs-Audit-Export (CSV)** | **Entparkt** (war ADR 0005 Phase 5, „Bedarf unklar") — Trigger ist eingetreten: EU-AI-Act-Nachweisbedarf (Art. 4). Jetzt Teil der Art.-4-Schärfung, ADR 0005 **Phase 6d**: Query aus `lib/training/compliance.ts` als `text/csv`-Response, inkl. Spalten für Compliance-Treiber, deklarierten Umfang (`estimatedMinutes`) und Lernkontroll-Ergebnis. |
