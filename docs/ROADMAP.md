...
| **Pflichtkurs-Audit-Export (CSV)** | ADR 0005 Phase 5, **parkiert** (Bedarf unklar). Query aus `lib/training/compliance.ts` als `text/csv`-Response wiederverwendbar (Name, Kurs, Status, Start-/Abschlussdatum, courseVersion-Snapshot, Zyklus) — kleiner, isolierter Zusatz, kein neuer Stack. Trigger: konkreter Auditor-/Nachweis-Bedarf, der über die Dashboard-Ansicht hinausgeht. |
